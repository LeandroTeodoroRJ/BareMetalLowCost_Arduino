 ...
 *  -- Add Step Motor with DM556 driver: pulse GPIO_10; direction GPIO_11
 *
...
void setup() {
 ...
 // pinMode(LED_BUILTIN, OUTPUT);
  ...
  /* Encoder Phase A Interrupt Config*/
  pinMode(GPIO_2, INPUT);
  attachInterrupt(digitalPinToInterrupt(GPIO_2), encoder_event, FALLING);
...
//  uint16_t old_sum = 32000;
  ...
  }else if (buffer_receiver[0] == 0x20){
    high = int(buffer_receiver[1]);
    low = int(buffer_receiver[2]);
  //  Serial.println(high);
  //  Serial.println(low);
    sum = high*256 + low;
    Serial.println(sum);        
    if(sum >= old_sum){
      ...
      step_value = sum - old_sum;       
      old_sum = sum;
  //    Serial.println(step_value);
      motor.setTarget(step_value + old_step); 
      old_step = step_value;    
    }else{  //ESTÁ VOLTANDO SOMANDO, IMPRIMIR AS VARIÁVEIS PARA ESTUDAR
      Serial.println("Motor ANTICLOCKWISE");
      motor.setDirection(ANTICLOCKWISE);
      step_value = sum - old_sum;
      old_sum = sum;
  //    Serial.println(step_value);
...
void output_update(){
  ...
  /* Deactivate Y0 and Y1 because usins Step Motor */
  /*
