...
void step_move(uint16_t high, uint16_t low){
  int16_t step_value;
  uint16_t sum;
  sum = high*256 + low;
//  Serial.println(sum);        
  if(sum >= 32000){   // 32000 is the midlle scale point (zero)
    motor.setDirection(CLOCKWISE);      
    step_value = sum - 32000;       
    motor.setTarget(step_value); 
  }else{  
...
void command_action(){
  if (buffer_receiver[0] == 0x23){
    ...
    step_move(int(buffer_receiver[1]), int(buffer_receiver[2]));
  }
}
